...
from tests.model_for_tests.reference.models import CarModel
...
class TestReferenceAndReverseRelations(TestReferencesSetUp):
    ...
    # TESTS
    def test_models_relation_attribute(self, gm_company):
        assert hasattr(gm_company,  CarModel.company.related_name)
    
    def test_relation_finds_correct_data(self, gm_company):
        car_model = gm_company.car_models.find().first()
        assert car_model['car_model'] == 'Camaro'
    
    def test_insert_not_allowed(self, gm_company):
        with pytest.raises(exceptions.OperationNotAllowed):
            gm_company.car_models.insert_one({})
    
    def test_insert_many_not_allowed(self, gm_company):
        with pytest.raises(exceptions.OperationNotAllowed):
            gm_company.car_models.insert_many({})
    
    def test_update_one_not_allowed(self, gm_company):
        with pytest.raises(exceptions.OperationNotAllowed):
            gm_company.car_models.update_one({}, {})
    
    def test_delete_one_not_allowed(self, gm_company):
        with pytest.raises(exceptions.OperationNotAllowed):
            gm_company.car_models.delete_one({})
    
    def test_add_new_car_model(self, gm_company, mongo: MongoDB):
        total_company_models = gm_company.car_models.all().count()  # Should be 1
        car = CarModel(company_id=gm_company.pk, make='Chevrolet', car_model='Corvette', color='red', year=2022)
        car.save()
